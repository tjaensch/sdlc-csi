...
11. **Scope discipline**: After applying your fix, run `git status --short`, `git diff --stat`, and `git diff --stat --cached`. If any tracked file appears that you did not intentionally edit — for example files modified by line-ending normalization (`.gitattributes`), editor auto-format, or trailing-whitespace tooling — revert it with `git restore --staged --worktree -- <file>` before finishing. If `git status --short` shows unintended untracked files, delete them separately before finishing. Your "What Changed" list must exactly match the files reported by `git diff --name-only` together with `git diff --name-only --cached`. Unintentional churn in unrelated files will cause reviewers to reject the PR.
