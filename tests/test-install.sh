...
assert_file_contains "$REPO1/.github/workflows/csi-run.yml" 'IGNORE_ISSUES=$(yq -r '\''(.ignore_issues // []) | join("\\u001f")'\'' "$CONFIG_FILE")'
assert_file_contains "$REPO1/.github/workflows/csi-run.yml" 'echo "ignore_issues=$IGNORE_ISSUES" >> "$GITHUB_OUTPUT"'
assert_file_contains "$REPO1/.github/workflows/csi-run.yml" '--- IGNORED ISSUES ---'
echo ""
